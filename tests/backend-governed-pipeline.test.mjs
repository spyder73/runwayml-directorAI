...
test('prompt linting flags image prompts that describe exclusions explicitly', () => {
  const { lintRunwayImagePrompt } = jiti('../src/lib/prompt-lint.ts');
...
test('scene outline prompts require cinematic atmosphere without title cards or collages', () => {
  ...
  assert.match(outlinePrompt, /visual tone and atmosphere/i);
  assert.match(outlinePrompt, /poetic, cinematic language/i);
...
test('fallback treatment outline creates scene rows from the approved story bucket', () => {
  ...
  assert.match(outline.scenes[0].imagePrompt, /Cinematic atmosphere for a movie scene/i);
  assert.match(outline.scenes[0].imagePrompt, /lighting, mood/i);
  assert.doesNotMatch(outline.scenes[0].imagePrompt, /Cinematic life-story frame|collage|title card|poster/i);
